...
def loadRegion(filename, region):
    '''
    get region from the file "area_params-tetrad.json"
    '''
    f = json.load(open(filename))
    return f[region]


def getBoundingBoxFromRegion(region):
    '''
    'region' comes from loadRegion()
    '''
    points = []
    for i, point in enumerate(region['Boundingbox']):
        points.append((i, float(point['Latitude']), float(point['Longitude'])))
    return points


def getRegionContainingPoint(lat, lon):
    regions = json.load(open('area_params-tetrad.json'))
    for name, info in regions.items():
        poly = getBoundingBoxFromRegion(info)
        if isQueryInBoundingBox(poly, lat, lon):
            return name, info
    return None
...
def applyCorrectionFactor2(factors, data_timestamp, data, sensor_type):
    """ this data comes from dict from area_params-tetrad.json file"""
    if sensor_type in factors.keys():
        factors_ls = factors[sensor_type]
    else:
        factors_ls = factors['Default']
    
    for factor in factors_ls:
        start = factor['starttime']
        end = factor['endtime']
        if start <= data_timestamp < end:
            return (data * factor['slope']) + factor['intercept']
