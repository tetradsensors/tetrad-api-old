...
def getAreaModels():
    if not hasattr(getAreaModels, 'area_models'):
        client = storage.Client()
        bucket = client.get_bucket('tetrad-resources')
        blob = bucket.get_blob('area_params.json')
        json_str = blob.download_as_string()
        json_data = json.loads(json_str)
        getAreaModels.area_models = buildAreaModelsFromJson(json_data)
    return getAreaModels.area_models
...
def applyCorrectionFactor(factors, data_timestamp, data, sensor_type, sensor_source=None, status=False):
    this_type = "0000"
...
def buildAreaElevationInterpolator(filename):
    data = loadmat('common/elevation/' + filename)
